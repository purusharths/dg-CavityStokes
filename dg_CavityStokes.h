...
public:
  ...
  std::vector<Vec<DIM, DataType>> eval_grad(const Vec<DIM, DataType> &x) const
  {
    ...
    // TODO EXERCISE B
    sol[0][0] = 2. * pi * this->max_v_ * std::sin(pi * x[0]) * std::cos(pi * x[0]) * std::sin(2. * pi * x[1]);
    sol[0][1] = 2. * pi * this->max_v_ * std::sin(pi * x[0]) * std::sin(pi * x[0]) * std::cos(2. * pi * x[1]);

    sol[1][0] = -2. * pi * this->max_v_ * std::cos(2. * pi * x[0]) * std::sin(pi * x[1]) * std::sin(pi * x[1]);
    sol[1][1] = -2. * pi * this->max_v_ * std::sin(2. * pi * x[0]) * std::sin(pi * x[1]) * std::cos(pi * x[1]);

    sol[DIM][0] = 0.;
    sol[DIM][1] = -2. * pi * std::sin(2. * pi * x[1]);
    // END EXERCISE B
    return sol;
  }
  ...
  std::vector<DataType> eval_laplace(const Vec<DIM, DataType> &x) const
  {
    ...
    // TODO EXERCISE B
    sol[0] = 2. * pi * this->max_v_ * (pi * std::cos(pi * x[0]) * std::cos(pi * x[0]) - pi * std::sin(pi * x[0]) * std::sin(pi * x[0])) * std::sin(2. * pi * x[1]) - 2. * pi * this->max_v_ * std::sin(pi * x[0]) * std::sin(pi * x[0]) * 2. * pi * std::sin(2. * pi * x[1]);

    sol[1] = 2. * pi * 2. * pi * this->max_v_ * std::sin(2. * pi * x[0]) * std::sin(pi * x[1]) * std::sin(pi * x[1]) - 2. * pi * this->max_v_ * std::sin(2. * pi * x[0]) * (pi * std::cos(pi * x[1]) * std::cos(pi * x[1]) - pi * std::sin(pi * x[1]) * std::sin(pi * x[1]));
    // END EXERCISE B
    return sol;
  }
...
{
  // material numbers
  // VelocityDirichletBC(int top_mat_number, DataType vel_x, DataType eps): top_mat_num_ (top_mat_number), eps_(eps),
  // vel_x_ (vel_x)
  VelocityDirichletBC()
  {
    ...
    for (int q = 0; q < num_q; ++q)
    {
      ...
      for (int i = 0; i < num_dof; ++i)
      {
        ...
        for (int j = 0; j < num_dof; ++j)
        {
          ...
          if (this->first_dof_for_var(DIM) <= i && i < this->last_dof_for_var(DIM) && this->first_dof_for_var(DIM) <= j && j < this->last_dof_for_var(DIM))
          {
            // TODO EXERCISE A
                  ...
                  LocalMatrix &lm)
  {
    ...
    for (int q = 0; q < num_q; ++q)
    {
      ...
      for (int i = 0; i < this->test().num_dofs_total(); ++i)
      {
        ...
        for (size_t var = 0; var < DIM; ++var)
        {
          phiV_i[var] = this->test().Phi(i, q, var);
          for (int d = 0; d != DIM; ++d)
          {
            DphiV_i(var, d) = this->test().grad_Phi(i, q, var)[d];
          }
        }
        ...
        for (int j = 0; j < this->trial().num_dofs_total(); ++j)
        {
          ...
          for (size_t var = 0; var < DIM; ++var)
          {
            phiV_j[var] = this->trial().Phi(j, q, var);
            for (int d = 0; d != DIM; ++d)
            {
              DphiV_j(var, d) = this->trial().grad_Phi(j, q, var)[d];
            }
          }
          ...
          // --------------------------------
          // ----- momentum - velocity ------
          if (this->test().first_dof_for_var(0) <= i && i < this->test().last_dof_for_var(pvar - 1) && this->trial().first_dof_for_var(0) <= j && j < this->trial().last_dof_for_var(pvar - 1))
          {
            if (this->do_sip_u_)
            {
              ...
              lm(i, j) += w_dS * nu_ * (c_penalty_u * (s_int + s_dir) * s_jump_i * s_jump_j * dot(phiV_i, phiV_j) - (s_int + s_dir) * s_jump_i * s_average * inner(phiV_i, DphiV_j, n_f) - (s_int + s_dir) * s_jump_j * s_average * inner(phiV_j, DphiV_i, n_f));
            }
          }

          // --------------------------------
          // ----- momentum - pressure ------
          if (this->test().first_dof_for_var(0) <= i && i < this->test().last_dof_for_var(pvar - 1) && this->trial().first_dof_for_var(pvar) <= j && j < this->trial().last_dof_for_var(pvar))
          {
          ...
          // ----------------------------------
          // ----- continuity - velocity ------
          if (this->test().first_dof_for_var(pvar) <= i && i < this->test().last_dof_for_var(pvar) && this->trial().first_dof_for_var(0) <= j && j < this->trial().last_dof_for_var(pvar - 1))
          {
            if (this->do_press_flux_)
            {
          ...
          // ----------------------------------
          // ----- continuity - pressure ------
          if (this->test().first_dof_for_var(pvar) <= i && i < this->test().last_dof_for_var(pvar) && this->trial().first_dof_for_var(pvar) <= j && j < this->trial().last_dof_for_var(pvar))
          {
      ...
      for (int i = 0; i < this->test().num_dofs_total(); ++i)
      {
        ...
        if (this->test().first_dof_for_var(0) <= i && i < this->test().last_dof_for_var(pvar - 1))
        {
          if (this->do_sip_u_)
          {
            ...
            //       + sigma / h * jump{u} * jump{v}
            //       + robin * u * v
            lv[i] += w_dS * nu_ * (s_dir * c_penalty_u * s_jump_i * dot(phiV_i, dirichlet_u) - s_dir * s_average * inner(dirichlet_u, DphiV_i, n_f));
          }
          if (this->do_press_flux_)
          {
          }
